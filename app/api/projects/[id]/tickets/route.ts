...
export async function GET(_: NextRequest, { params }: { params: { id: string }}) {
  ...
  const snap = await db.collection("projects").doc(params.id).collection("tickets").orderBy("createdAt","desc").get();
...
export async function POST(req: NextRequest, { params }: { params: { id: string }}) {
  ...
  const { title, description = "", assignee = null, priority = "normal" } = body || {};
  if (!title) return NextResponse.json({ error: "title required" }, { status: 400 });

  const now = new Date();
  const doc = {
    title, description,
    assignee, priority, status: "open",
    createdAt: now, createdBy: user.uid,
  };
  const ref = await db.collection("projects").doc(params.id).collection("tickets").add(doc);
