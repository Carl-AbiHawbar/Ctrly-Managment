...
export async function PATCH(req: NextRequest, { params }: { params: { id: string, taskId: string }}) {
  ...
  await db.collection("projects").doc(params.id).collection("tasks").doc(params.taskId).update(patch);
  const updated = await db.collection("projects").doc(params.id).collection("tasks").doc(params.taskId).get();
